...
"ldots" : b'',
...
class Pdf_viewer:
    def __init__(self):
        ...
        self.base_values = {"page_number": 0, "zoom": 1.2, "has_empty": "y", "text": False, "fontsize": 20, "path" : "/home/marijke/Documenten/Rekening CoderDojo - 2.pdf"}
        self.path = self.base_values["path"]
        self.root = tk.Tk()
        #self.font = Font(root=root.master, size=25 , weight="bold")
    ...
    def make_page_image(self, page_number, zoom):
        ...
        pix = self.doc[page_number].get_pixmap(matrix = m)
    ...
    def make_toolbar(self):
        ...
        line_button = tk.Button(toolbar, text = 'line', relief='groove',
            command=lambda: 0)
        line_button.pack(side=('left'))

        freedraw_button = tk.Button(toolbar, text = 'draw', relief='groove',
            command=lambda: 0)
        freedraw_button.pack(side=('left'))

        marker_button = tk.Button(toolbar, text = 'marker', relief='groove',
            command=lambda: 0)
        marker_button.pack(side=('left'))

        text_write_button = tk.Button(toolbar, text = 'text', relief='groove',
            command=lambda: 0)
        text_write_button.pack(side=('left'))

        var = tk.StringVar(value = [4,8,12,16,20,24])
        text_write_button_listbox = tk.Listbox(toolbar, listvariable = var, height = 1, width = 2)
        text_write_button_listbox.bind('<<ListboxSelect>>', lambda e: self.set_fontsize(e.widget.get(e.widget.curselection()[0])))
        text_write_button_listbox.see(4)
        text_write_button_listbox.pack(side=('left'))


        toolbar.pack(side='top', fill='x')
        toolbar2 = tk.Frame(self.frame)
        toolbar2.pack(side='top', fill='x')
        control_centre = Symbol_control_center(toolbar2, toolbar, self.frame)
    ...
    def put_photoimage_in_list(self, img, page_number, labels_image):
        if labels_image[page_number] == None:
            labels_image[page_number] = ImageTk.PhotoImage(img)
        else:
            labels_image[page_number].paste(img)
    def set_image(self, page_number):
        img = self.make_page_image(page_number, self.zoom)
        #self.remove_redundant(i)
        self.put_photoimage_in_list(img, page_number,self.page_image_list)
    ...
    def save(self):
        ...
        self.save_info(str(self.cursor_page_number()), "page_number")
        self.save_info(str(self.zoom), "zoom")
        self.save_info(self.path, "path")
    ...
    def create_latex_widget(self, event = None):
        ...
        self.page_widgets[page_number][-1].make_active(event, True)
    ...
    def remove_widgets_on_page(self, page_number, zoom=None):
        ...
        for widget in self.page_widgets[page_number]:
            ...
            if widget_string not in self.page_widgets_save_object[page_number]:
...
class Textmanager:
    def __init__(self):
        ...
        self.symbol_dict = {
        'wan(': 'widehat', 'int': 'int', 'wbar(': 'widebar', 'lan': 'leftangle', 'ran': 'rightangle', 'perp': 'perp', 'neq': 'neq', 'leqap': 'lessapprox', 'geqap': 'gtrapprox', 'teq': 'triangleeq', 'el': 'backepsilon', 'sbeq': 'subseteq', 'speq': 'supseteq', 'sbneq': 'subsetneq', 'spneq': 'supsetneq', 'lnap': 'lnapprox', 'gnap': 'gnapprox', 'lla': 'longleftarrow', 'lra': 'leftrightarrow', 'llra': 'longleftrightarrow', 'Lla': 'Longleftarrow', 'Lra': 'Longrightarrow', 'Llra': 'Longleftrightarrow', 'mapt': 'longmapsto', 'ldots': 'ldots', 'vdots': 'vdots', 'deqd': 'doteqdot', 'deq': 'doteq', 'par': 'parallel', 'asy': 'asymp', 'oti': 'otimes', 'vec(': 'vec', 'ntl': 'ntriangleleft', 'nleq': 'nleq', 'div': 'div', 'npa': 'nparallel', 'La': 'Lambda', 'lll': 'lll', 'oiint': 'oiint', 'ngeq': 'ngeq', 'Th': 'Theta', 'bsq': 'blacksquare', 'neg': 'neg', 'sum': 'sum', 'bowt': 'bowtie', 'btr': 'blacktriangleright', 'leq': 'leq', 'ggg': 'ggg', 'lneqq': 'lneqq', 'Na': 'BbbN', 'Co': 'BbbC', 'In': 'BbbZ', 'Re': 'BbbR', 'Ra': 'Rightarrow', 'vtr': 'vartriangleright', 'and': 'wedge', 'ch': 'chi', 'prec': 'prec', 'nseq': 'nsubseteq', 'nin': 'notin', 'tri': 'bigtriangleup', 'ups': 'upsilon', 'cap': 'cap', 'mu': 'mu', 'tau': 'tau', 'in': 'in', 'eqsim': 'eqsim', 'gnsim': 'gnsim', 'tleq': 'triangleq', 'sqseq': 'sqsubseteq', 'mapf': 'mapsfrom', 'sim': 'sim', 'nequ': 'nequiv', 'succ': 'succ', 'vda': 'vdash', 'upa': 'uparrow', 'nsim': 'nsim', 'eqqq': 'Equiv', 'app': 'approx', 'nsucc': 'nsucc', 'eqq': 'equiv', 'Uni': 'bigcup', 'iint': 'iiint', 'And': 'bigwedge', 'sb': 'subset', 'leqq': 'leqq', 'mapup': 'mapsup', 'nl': 'nless', 'nap': 'napprox', 'ast': 'ast', 'Si': 'Sigma', 'ntleq': 'ntrianglelefteq', 'nex': 'nexists', 'tim': 'times', 'man': 'measuredangle', 'adots': 'adots', 'check': 'checkmark', 'la': 'lambda', 'xi': 'xi', 'var': 'varphi', 'De': 'Delta', 'io': 'iota', 'nla': 'nleftarrow', 'sps': 'supset', 'tl': 'triangleleft', 'ntreq': 'ntrianglerighteq', 'Xi': 'Xi', 'precceq': 'preccurlyeq', 'nlra': 'nleftrightarrow', 'Ph': 'Phi', 'an': 'angle', 'uda': 'updownarrow', 'be': 'beta', 'rh': 'rho', 'al': 'alpha', 'redo': 'acwopencirclearrow', 'mid': 'mid', 'nmid': 'nmid', 'nsp': 'nsupset', 'si': 'sigma', 'dot': 'dot', 'apeq': 'approxeq', 'ceqsucc': 'curlyeqsucc', 'Psi': 'Psi', 'yen': 'yen', 'tr': 'triangleright', 'precsim': 'precsim', 'inf': 'infty', 'geq': 'geq', 'precnsim': 'precnsim', 'ncong': 'ncong', 'da': 'downarrow', 'ceq': 'circeq', 'lc': 'lceil', 'rc': 'rceil', 'oiiint': 'oiiint', 'lf': 'lfloor', 'rf': 'rfloor', 'ddots': 'ddots', 'ex': 'exists', 'ub(': 'underbar', 'Pi': 'Pi', 'cpro': 'coprod', 'gsim': 'gtrsim', 'gneqq': 'gneqq', 'betw': 'between', 'th': 'theta', 'Com': 'complement', 'na': 'nabla', 'dplus': 'dotplus', 'eta': 'eta', 'fa': 'forall', 'squ': 'sqcup', 'sqsp': 'sqsupset', 'mapto': 'mapsto', 'btd': 'bigtriangledown', 'sqspeq': 'sqsupseteq', 'propto': 'propto', 'pi': 'pi', 'dots': 'dots', 'nra': 'nrightarrow', 'Doteq': 'Doteq', 'sqdo': 'sqcap', 'ka': 'kappa', 'nprec': 'nprec', 'con': 'cong', 'sq(': 'sqrt', 'oint': 'oint', 'eqdef': 'eqdef', 'phi': 'phi', 'Ga': 'Gamma', 'ceqprec': 'curlyeqprec', 'qeq': 'questeq', 'uni': 'cup', 'cdots': 'cdots', 'hat(': 'hat', 'ps': 'psi', 'ntr': 'ntriangleright', 'unidot': 'cupdot', 'sqsb': 'sqsubset', 'cdot': 'cdot', 'Om': 'Omega', 'nsub': 'nsubset', 'ze': 'zeta', 'dddot(': 'dddot', 'bcir': 'bigcirc', 'cir': 'circ', 'ocir(': 'ocirc', 'pro': 'prod', 'treq': 'trianglerighteq', 'nwa': 'nwarrow', 'ddddot(': 'ddddot', 'succeq': 'succeq', 'ga': 'gamma', 'nspeq': 'nsupseteq', 'bcap': 'bigcap', 'de': 'delta', 'vecl(': 'overleftarrow', 'geqq': 'geqq', 'ddot(': 'ddot', 'oplu': 'oplus', 'ni': 'ni', 'ne': 'ne', 'leqgtr': 'lesseqgtr', 'obar': 'obar', 'nu': 'nu', 'omin': 'ominus', 'om': 'omega', 'Ss': 'Subset', 'to': 'to', 'emp': 'emptyset', 'bopl': 'bigoplus', 'bodot': 'bigodot', 'preceq': 'preceq', 'ep': 'epsilon', 'bot': 'bigotimes'}
    ...
    def make_frac(self, text, sym = "/", transitions = {}):
        ...
        return self.make_frac(self.frac_pair(numerator_divisor_pair, text,sym, transitions),sym,transitions)
    def frac_pair(self,pair, text,sym,  transitions):
        ...
        if sym == "/":
            ...
            new_str = "{num}{sym}{div}".format(sym=self.replace_symbol,num = "{" + numerator + "}", div = "{" + divisor+ "}")
    ...
    def replace_bracket_function(self, func, text, transitions):
        ...
        if end_brac == -1:
            ...
            new_str = "{sym}{exp}{p}".format(sym = self.symbol_dict[func],exp = exp2 ,  p = "{}")
        ...
        new_str = "{sym}{exp}{p}".format(sym = self.symbol_dict[func],exp = exp2 ,  p = "{" + placeholder[:-1]+ "}")
        transitions[self.replace_useless(new_str)] = self.replace_useless(old_str)
        new_str = "(" + self.special_char +self.special_char+ new_str + self.special_char+self.special_char + ")"
        return self.replace_bracket_function(func, text.replace(old_str, new_str,1),transitions)
...
class Symbol_control_center:
    ...
    def make_categories(self):
        ...
        self.list.append(Symbol_Category("GriekseLetters", griekse_letters, self))
        ...
        self.list.append(Symbol_Category("Driehoek", triangle, self))
        ...
        self.list.append(Symbol_Category("Letters", letterachtige_symbolen, self))
        self.list.append(Symbol_Category("Logica", logica, self))
        self.list.append(Symbol_Category("Meetkunde", meetkunde, self))
...
class Symbol_Category():
    ...
    def open_symbols(self):
        self.destroy_all()
        self.frame = tk.Frame(self.root)
        self.symbols = [Symbol_button(self.frame, code, self.root2) for code in self.code_list]
        for s in self.symbols:
...
class Latex_Widget():
    def __init__(self, textmanager,pdfv, page_number, fontsize=20, is_active = False):
        ...
        self.button_font = tk.font.Font(size=int(fontsize/2.5))
        self.fontsize = fontsize
        self.fontsize_save = fontsize
        self.dpi = 100
        self.fig = plt.Figure(figsize=(self.fontsize/200, self.fontsize/300), dpi=self.dpi)
        self.input = ""
    ...
    def entered_widget(self, event):
        ...
        if self.y-self.fontsize<0:
            ...
            self.button_frame.place(x=self.x,y=self.y-self.fontsize_save)
    ...
    def left_widget(self, event):
        ...
        W=self.button_frame.winfo_containing(temp[0],temp[1]).winfo_parent()
        if self.pdfv.root.nametowidget(W).winfo_class() != "Canvas":
    ...
    def make_transparent(self):
        hwnd = self.frame_latex.winfo_id()
    ...
    def process_input(self, text, event=None):
        ...
        self.fig.savefig("test1.png", bbox = self.bbox)
        if self.is_active and self.latex_canvas._tkcanvas.winfo_width != int(self.bbox.x1):
            self.place_entry()
        """if event !=None and event.type == '3':
            pass
        else:"""
        self.latex_canvas._tkcanvas.configure(width = int(self.bbox.x1), height = int(self.bbox.y1))
        #self.latex_canvas.get_tk_widget().configure(width = int(self.bbox.x1), height = int(self.bbox.y1))
    def make_text(self,string):
        if self.text != None:
            self.text.remove()
        self.text=self.fig.text(0, -2/self.dpi, string + " ",va="bottom",  fontsize=self.fontsize,bbox=dict(facecolor='white', alpha=0.1))
    def get_input(self):
    ...
    def make_sub_figures(self):
        ...
        print(hierarchy)
        j_=0
        #for j,hier in enumerate(hierarchy[0][1:]):
        j=1
        while j<len(contours):
            ...
            if hier[3] != 0:
                ...
                if char==self.textmanager.special_char:
                    for i_, c in enumerate(self.input, start = i):
                        if not c.isalpha():
                            finds.append(([x,y,w,h],i_-1))
                            i=i_
                            break
                else:
                    i_=i
                    for s in self.textmanager.multi_symbols_list:
                        if self.input.find(s,i) == i:
                            i+=len(s)
                            finds.append(([x,y,w,h],i))
                            break
                    if i_==i:
                        print(i)
                        if " " != self.input[i]:
        ...
        rects = sorted(rects)
        print("------------------------------")
        #print(len(rects),len(self.input))"

        total = ""
        for i in range(1, len(finds)):
            total += self.input[finds[i][1]]
        print(total)
    def fig2img(self):
        buf = io.BytesIO()
        extent = self.bbox.transformed(self.fig.dpi_scale_trans.inverted())
